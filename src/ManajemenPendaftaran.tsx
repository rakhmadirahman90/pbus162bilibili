    ...
    try {
      ...
      if (error) {
        if (error.message.includes('check_kategori_atlet')) {
          throw new Error('Kategori Atlet harus berisi "Senior" atau "Atlet Muda"');
        }
        throw error;
      }
    ...
    <div className="min-h-screen bg-slate-50 p-4 md:p-8 font-sans">
                    ...
                    <td className="px-8 py-6">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <a href={`https://wa.me/${item.whatsapp}`} target="_blank" rel="noreferrer" className="flex items-center gap-1.5 text-blue-600 font-bold text-xs mt-0.5 hover:underline">
      ...
      {previewImage && (
        <div className="fixed inset-0 z-[110] flex flex-col items-center justify-center p-4 bg-slate-900/90 backdrop-blur-md animate-in fade-in duration-300">
          <div className="flex gap-4 mb-4 bg-white/10 p-2 rounded-full backdrop-blur-md border border-white/20">
            <button onClick={() => setZoomScale(s => Math.max(0.5, s - 0.25))} className="p-2 text-white hover:bg-white/20 rounded-full transition-all">
              <ZoomOut size={24} />
            </button>
            <span className="text-white font-bold flex items-center px-2">{Math.round(zoomScale * 100)}%</span>
            <button onClick={() => setZoomScale(s => Math.min(3, s + 0.25))} className="p-2 text-white hover:bg-white/20 rounded-full transition-all">
              <ZoomIn size={24} />
            </button>
            <button onClick={() => { setPreviewImage(null); setZoomScale(1); }} className="p-2 text-white hover:bg-rose-500 rounded-full transition-all ml-2">
              <X size={24} />
            </button>
          </div>
          
          <div className="relative max-w-4xl w-full flex justify-center overflow-auto hide-scrollbar">
            <img 
              src={previewImage} 
              alt="Preview" 
              className="rounded-3xl shadow-2xl transition-transform duration-200 ease-out origin-center border-4 border-white/10" 
