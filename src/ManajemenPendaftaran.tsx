...
export default function ManajemenPendaftaran() {
  ...
  // --- FITUR BARU: STATISTIK DINAMIS (Sinkron dengan data tabel) ---
  const stats = {
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        // Handle constraint database jika ada kesalahan input
        if (error.message.includes('check_kategori_atlet')) {
          throw new Error('Gagal: Kategori Atlet harus "Senior" atau "Atlet Muda"');
        }
        throw error;
      }
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-4 md:p-8 font-sans">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        {/* STATS CARDS - 100% ASLI + LOGIKA DINAMIS */}
          ...
          <div className="bg-white rounded-[2.5rem] w-full max-w-2xl max-h-[90vh] overflow-hidden shadow-2xl flex flex-col">
            ...
            <form onSubmit={handleUpdate} className="p-8 overflow-y-auto space-y-6">
              ...
              <div className="grid md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Kategori Permainan</label>
                  <input className="w-full px-5 py-4 bg-slate-50 rounded-2xl border-none focus:ring-2 focus:ring-blue-600 transition-all font-bold text-slate-700" value={editingItem.kategori} onChange={e => setEditingItem({...editingItem, kategori: e.target.value})} required />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Kategori Atlet (DB Sync)</label>
                  <select 
      ...
      {previewImage && (
        <div className="fixed inset-0 z-[110] flex flex-col items-center justify-center p-4 bg-slate-900/90 backdrop-blur-md animate-in fade-in duration-300">
          ...
          {/* Zoom Toolbar */}
          <div className="flex items-center gap-4 bg-white/10 p-2 rounded-full backdrop-blur-md mb-4 border border-white/20">
            <button onClick={() => setZoomScale(s => Math.max(0.5, s - 0.25))} className="p-2 text-white hover:bg-white/20 rounded-full">
              <ZoomOut size={20} />
            </button>
            <span className="text-white font-bold text-xs w-12 text-center">{Math.round(zoomScale * 100)}%</span>
            <button onClick={() => setZoomScale(s => Math.min(3, s + 0.25))} className="p-2 text-white hover:bg-white/20 rounded-full">
              <ZoomIn size={20} />
            </button>
            <div className="w-px h-4 bg-white/20 mx-1"></div>
            <button onClick={() => { setPreviewImage(null); setZoomScale(1); }} className="p-2 text-white hover:bg-rose-500 rounded-full">
              <X size={20} />
            </button>
          </div>
