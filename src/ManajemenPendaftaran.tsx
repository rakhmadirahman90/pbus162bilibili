      ...
      <div className="max-w-[1400px] mx-auto px-4 py-10 md:px-8">
        ...
        <header className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6 mb-12">
          <div className="space-y-2">
            ...
            <p className="text-slate-500 font-medium max-w-md">Panel manajemen data pendaftaran atlet secara real-time dan terintegrasi.</p>
          </div>

          <div className="flex flex-wrap items-center gap-3">
            <div className="px-5 py-3 bg-white border border-slate-200 rounded-2xl shadow-sm">
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-0.5 text-center">Data Masuk</p>
        ...
        <section className="mb-10 group">
          <div className="relative overflow-hidden rounded-[2rem] bg-white border border-slate-200 shadow-xl shadow-slate-100/50 transition-all focus-within:ring-4 focus-within:ring-blue-50 focus-within:border-blue-200">
            ...
            <input 
              type="text"
              placeholder="Cari nama atlet atau kota domisili..."
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50/50 border-b border-slate-100">
                  <th className="pl-8 pr-4 py-6 font-bold uppercase text-[11px] text-slate-400 tracking-widest w-20 text-center">No</th>
                  ...
                  <th className="px-8 py-6 font-bold uppercase text-[11px] text-slate-400 tracking-widest">WhatsApp</th>
                  <th className="px-8 py-6 font-bold uppercase text-[11px] text-slate-400 tracking-widest">Domisili</th>
                  <th className="px-8 py-6 font-bold uppercase text-[11px] text-slate-400 tracking-widest text-right">Aksi</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-50">
                {loading && registrants.length === 0 ? (
                  <tr><td colSpan={6} className="py-32 text-center text-slate-400 font-bold uppercase tracking-[0.3em] animate-pulse">Menghubungkan Database...</td></tr>
                ) : currentItems.length === 0 ? (
                  <tr><td colSpan={6} className="py-32 text-center text-slate-400 font-bold uppercase tracking-[0.2em]">Tidak Ada Data Tersedia</td></tr>
                  ...
                  <tr key={item.id} className="hover:bg-blue-50/30 transition-all duration-300 group">
                    ...
                    <td className="px-8 py-6">
                      <div className="flex items-center gap-5">
                        <div 
                          ...
                          className="relative w-14 h-14 rounded-2xl bg-slate-100 border-2 border-white shadow-md overflow-hidden flex-shrink-0 cursor-zoom-in group-hover:rotate-3 transition-transform"
                        >
                          {item.foto_url ? (
                            <img src={item.foto_url} className="w-full h-full object-cover" alt={item.nama} />
                          ) : (
                            <User className="m-auto mt-3 text-slate-300" size={28} />
                          )}
                        </div>
                        <div className="space-y-0.5">
                          <h4 className="font-bold text-slate-900 text-base leading-tight group-hover:text-blue-600 transition-colors uppercase tracking-tight">{item.nama}</h4>
                    ...
                    <td className="px-8 py-6">
                      <span className="inline-flex items-center bg-white border border-slate-200 text-slate-700 px-3 py-1 rounded-lg text-[10px] font-black uppercase italic tracking-wider shadow-sm">
                        {item.kategori}
                      </span>
                    </td>
                    ...
                    <td className="px-8 py-6">
                      ...
                      >
                        <div className="w-7 h-7 rounded-lg bg-green-50 flex items-center justify-center text-green-500 group-hover:bg-green-100 transition-colors">
                          <Phone size={12} />
                        </div>
                        <span className="text-xs tracking-wide">{item.whatsapp}</span>
                      </a>
                    </td>

                    <td className="px-8 py-6">
                      <div className="flex items-center gap-2 font-bold text-slate-500 uppercase italic text-xs">
                    ...
                    <td className="px-8 py-6">
                      <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button 
                          ...
                          className="p-2.5 bg-white border border-slate-200 text-slate-400 rounded-xl hover:text-blue-600 hover:border-blue-200 hover:shadow-lg transition-all"
                        >
                          <Edit3 size={16} />
                        </button>
                        <button 
                          onClick={() => handleDelete(item.id, item.nama, item.foto_url)}
                          className="p-2.5 bg-white border border-slate-200 text-slate-400 rounded-xl hover:text-rose-600 hover:border-rose-200 hover:shadow-lg transition-all"
                        >
                          <Trash2 size={16} />
                        </button>
                      </div>
        ...
        {/* PAGINATION */}
        <footer className="flex flex-col sm:flex-row justify-between items-center gap-4 px-8 py-5 bg-white rounded-3xl border border-slate-200 shadow-lg shadow-slate-100">
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em]">Halaman {currentPage} Dari {totalPages || 1}</p>
          <div className="flex items-center gap-2">
            <button 
              onClick={() => setCurrentPage(p => Math.max(p - 1, 1))}
              disabled={currentPage === 1}
              className="p-2.5 border border-slate-200 rounded-xl disabled:opacity-20 hover:bg-slate-50 transition-colors"
            >
              <ChevronLeft size={18} />
            </button>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white w-full max-w-2xl rounded-[3rem] shadow-2xl overflow-hidden animate-in zoom-in duration-300">
            <div className="px-10 py-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
              <div>
                <h2 className="text-2xl font-black text-slate-900 uppercase italic tracking-tighter">Profil Editor</h2>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Update data pendaftaran atlet</p>
              </div>
            ...
            <form onSubmit={handleUpdate} className="p-10 space-y-8 max-h-[75vh] overflow-y-auto">
              <div className="flex flex-col md:flex-row items-center gap-8">
                <div className="relative group">
                  <div className="w-40 h-40 rounded-[2.5rem] bg-slate-100 border-4 border-white shadow-2xl overflow-hidden relative">
                    {editingItem.foto_url ? <img src={editingItem.foto_url} className="w-full h-full object-cover" alt="Editor" /> : <User size={60} className="m-auto mt-10 text-slate-200" />}
                  ...
                  <label className="absolute -bottom-2 -right-2 p-4 bg-blue-600 text-white rounded-2xl shadow-xl cursor-pointer hover:bg-slate-900 transition-all hover:scale-110 shadow-blue-200">
                    <Camera size={20} />
                ...
                <div className="flex-1 space-y-6 w-full">
                   <div className="space-y-2">
                      <label className="text-[10px] font-black uppercase text-slate-400 tracking-[0.2em] ml-1">Nama Lengkap Atlet</label>
                      <input 
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase text-slate-400 tracking-[0.2em] ml-1">Nomor WhatsApp</label>
                  <input className="w-full px-6 py-4 bg-slate-50 border border-slate-200 rounded-2xl font-bold" value={editingItem.whatsapp} onChange={e => setEditingItem({...editingItem, whatsapp: e.target.value})} required />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase text-slate-400 tracking-[0.2em] ml-1">Kategori Tanding</label>
                  <select className="w-full px-6 py-4 bg-slate-50 border border-slate-200 rounded-2xl font-bold appearance-none cursor-pointer" value={editingItem.kategori} onChange={e => setEditingItem({...editingItem, kategori: e.target.value})}>
                ...
                <div className="md:col-span-2 space-y-2">
                  ...
                  <input className="w-full px-6 py-4 bg-slate-50 border border-slate-200 rounded-2xl font-bold uppercase" value={editingItem.domisili} onChange={e => setEditingItem({...editingItem, domisili: e.target.value})} required />
                </div>
              </div>

              <div className="pt-6 flex gap-4">
                <button type="button" onClick={() => setIsEditModalOpen(false)} className="flex-1 py-5 border-2 border-slate-100 rounded-[1.5rem] font-bold uppercase text-xs tracking-widest text-slate-400 hover:bg-slate-50 transition-all">Batal</button>
                <button type="submit" disabled={isSaving || uploading} className="flex-[2] py-5 bg-blue-600 text-white rounded-[1.5rem] font-bold uppercase text-xs tracking-[0.2em] shadow-xl shadow-blue-200 hover:bg-slate-900 transition-all flex items-center justify-center gap-3">
      ...
      {previewImage && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-slate-900/90 backdrop-blur-xl animate-in fade-in duration-300" onClick={() => setPreviewImage(null)}>
          <div className="relative max-w-2xl w-full animate-in zoom-in duration-300">
            ...
            <button className="absolute -top-12 right-0 text-white flex items-center gap-2 font-bold uppercase text-[10px] tracking-widest hover:text-blue-400 transition-colors">
              <X size={20}/> Klik dimana saja untuk tutup
            </button>
          </div>
        </div>
      )}
