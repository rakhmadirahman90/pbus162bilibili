...
export default function ManajemenPendaftaran() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>, mode: 'add' | 'edit') => {
    ...
    try {
      ...
      Toast.fire({ icon: 'success', title: 'Foto diunggah' });
    } catch (error: any) {
  ...
  const exportToPDF = () => {
    ...
    autoTable(doc, { head: [['No', 'Nama', 'Gender', 'Kat', 'WA', 'Kota']], body: tableData });
  ...
  const downloadTemplate = () => {
    ...
    XLSX.writeFile(wb, "Template_Atlet.xlsx");
  };
    ...
    <div className="min-h-screen bg-slate-50 pb-10">
      <div className="max-w-[1400px] mx-auto px-4 py-6">
        ...
        <div className="mt-6 flex justify-between items-center bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
           <p className="text-[10px] font-bold text-slate-400 uppercase">Total Data: {filteredData.length}</p>
           <div className="flex gap-2">
             <button disabled={currentPage === 1} onClick={() => setCurrentPage(p => p - 1)} className="p-2 bg-slate-50 border rounded-lg disabled:opacity-50 hover:bg-white"><ChevronLeft size={16}/></button>
             <button disabled={currentPage === totalPages} onClick={() => setCurrentPage(p => p + 1)} className="p-2 bg-slate-50 border rounded-lg disabled:opacity-50 hover:bg-white"><ChevronRight size={16}/></button>
           </div>
        </div>
      </div>
      ...
      {previewImage && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-slate-900/90 backdrop-blur-md" onClick={() => setPreviewImage(null)}>
          <div className="relative max-w-xl w-full">
            <button className="absolute -top-12 right-0 text-white hover:text-rose-500 flex items-center gap-2 font-black uppercase text-[10px] tracking-widest transition-colors">
              Tutup <X size={20} />
            </button>
