...
export default function ManajemenPendaftaran() {
  ...
  // STATE UNTUK STATISTIK OTOMATIS
  ...
  // FUNGSI BARU: Hitung Statistik Terkoneksi Tabel
  const calculateStats = (data: Registrant[]) => {
    const total = data.length;
    const senior = data.filter(item => item.kategori_atlet?.toLowerCase() === 'senior').length;
    const muda = data.filter(item => item.kategori_atlet?.toLowerCase() === 'atlet muda').length;
    setStats({ total, senior, muda });
  };
  ...
  const fetchRegistrants = async () => {
    try {
      ...
      if (data) {
        setRegistrants(data);
        calculateStats(data); // Hubungkan data ke angka statistik
      }
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-4 md:p-8 font-sans">
            ...
            <div className="flex gap-2">
              ...
              {[...Array(totalPages)].map((_, i) => (
                <button
                  ...
                  className={`w-11 h-11 rounded-xl font-bold text-xs transition-all ${currentPage === i + 1 ? 'bg-blue-600 text-white shadow-lg shadow-blue-200' : 'bg-white text-slate-400 border border-slate-200'}`}
                >
                  {i + 1}
                </button>
              ))}
          ...
          <div className="bg-white rounded-[2.5rem] w-full max-w-2xl max-h-[90vh] overflow-hidden shadow-2xl flex flex-col border border-white/20">
            ...
            <form onSubmit={handleUpdate} className="p-8 overflow-y-auto space-y-6">
              <div className="flex flex-col items-center gap-4 mb-8">
                ...
                <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Ganti Foto Profil</p>
              </div>
              ...
              <div className="grid md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  ...
                  <select className="w-full px-5 py-4 bg-slate-50 rounded-2xl border-none focus:ring-2 focus:ring-blue-600 font-bold text-slate-700" value={editingItem.kategori} onChange={e => setEditingItem({...editingItem, kategori: e.target.value})}>
                ...
                <div className="space-y-2">
                  ...
                  <select className="w-full px-5 py-4 bg-slate-50 rounded-2xl border-none focus:ring-2 focus:ring-blue-600 font-bold text-slate-700" value={editingItem.kategori_atlet} onChange={e => setEditingItem({...editingItem, kategori_atlet: e.target.value})}>
                ...
                <div className="space-y-2">
                  ...
                  <select className="w-full px-5 py-4 bg-slate-50 rounded-2xl border-none focus:ring-2 focus:ring-blue-600 font-bold text-slate-700" value={editingItem.jenis_kelamin} onChange={e => setEditingItem({...editingItem, jenis_kelamin: e.target.value})}>
      ...
      {/* LIGHTBOX PREVIEW DENGAN ZOOM FITUR */}
      {previewImage && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-slate-900/90 backdrop-blur-md animate-in fade-in duration-300">
          ...
          <div className="absolute top-8 left-1/2 -translate-x-1/2 flex items-center gap-2 bg-white/10 p-2 rounded-2xl backdrop-blur-xl border border-white/20 z-[120]">
             <button onClick={() => setZoomScale(s => Math.max(0.5, s - 0.25))} className="p-3 text-white hover:bg-white/10 rounded-xl transition-all">
                <ZoomOut size={20} />
             </button>
             <span className="text-white font-black text-xs w-16 text-center tracking-widest">{Math.round(zoomScale * 100)}%</span>
             <button onClick={() => setZoomScale(s => Math.min(3, s + 0.25))} className="p-3 text-white hover:bg-white/10 rounded-xl transition-all">
                <ZoomIn size={20} />
             </button>
             <div className="w-px h-6 bg-white/20 mx-1"></div>
             <button onClick={() => setZoomScale(1)} className="p-3 text-white hover:bg-white/10 rounded-xl transition-all">
                <Maximize size={20} />
             </button>
          </div>
          ...
          <div className="relative w-full h-full flex items-center justify-center overflow-hidden">
            <button 
              className="absolute top-12 right-12 text-white hover:text-rose-500 transition-colors flex items-center gap-2 font-black uppercase text-[10px] tracking-widest z-[130]"
              onClick={() => {
...
// Catatan: Pastikan library 'framer-motion' terinstall untuk fitur drag & zoom yang halus.
