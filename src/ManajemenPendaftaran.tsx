...
export default function ManajemenPendaftaran() {
  ...
  // New Registrant State for Add Modal
  const [newRegistrant, setNewRegistrant] = useState<Partial<Registrant>>({
    nama: '',
    whatsapp: '',
    domisili: '',
    ...
    reader.onload = async (evt) => {
      try {
        ...
        const formattedData = data.map(item => ({
          ...
          domisili: item.Domisili || item.domisili || '',
        }));

        const { error } = await supabase.from('pendaftaran').insert(formattedData);
        if (error) throw error;
        
        notify("✅ Berhasil mengimport " + formattedData.length + " data!");
        fetchData();
      } catch (err: any) {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>, mode: 'add' | 'edit') => {
    ...
    try {
      ...
      if (mode === 'edit') {
        ...
        setNewRegistrant(prev => ({ ...prev, foto_url: publicUrl }));
      }
  ...
  const handleAddSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { data, error } = await supabase
        ...
        .insert([{
          ...newRegistrant,
          nama: (newRegistrant.nama || '').toUpperCase()
        }])
        .select();

      if (error) throw error;
      
      notify("✅ Atlet berhasil ditambahkan!");
      setIsAddModalOpen(false);
      setNewRegistrant({ nama: '', whatsapp: '', domisili: '', kategori: 'Dewasa / Umum', jenis_kelamin: 'Putra', foto_url: '' });
      // State updated by Realtime channel or manual fetch
    } catch (err: any) {
      alert(err.message);
    } finally {
      setIsSaving(false);
      ...
      <div className="max-w-[1400px] mx-auto px-4 py-4 md:px-8">
        ...
        <header className="flex flex-col lg:flex-row justify-between items-center gap-4 mb-4">
          ...
          <div className="flex flex-wrap items-center justify-center gap-2">
            <button onClick={() => setIsAddModalOpen(true)} className="flex items-center gap-2 bg-blue-600 text-white px-3 py-2.5 rounded-xl font-bold text-[9px] tracking-widest hover:bg-slate-900 transition-all active:scale-95 shadow-lg shadow-blue-100">
              <Plus size={14} /> TAMBAH ATLET
            </button>
            <label className="flex items-center gap-2 bg-indigo-600 text-white px-3 py-2.5 rounded-xl font-bold text-[9px] tracking-widest hover:bg-indigo-700 cursor-pointer transition-all active:scale-95 shadow-lg shadow-indigo-100">
              ...
              <div className="flex items-center gap-6 mb-2">
                <div className="relative">
                  <div className="w-20 h-20 rounded-2xl bg-slate-100 border-2 border-white shadow-md overflow-hidden flex-shrink-0">
                    { (isEditModalOpen ? editingItem?.foto_url : newRegistrant.foto_url) ? (
                      <img src={isEditModalOpen ? editingItem?.foto_url : newRegistrant.foto_url} className="w-full h-full object-cover object-top" alt="preview" /> 
                    ) : (
                ...
                <div className="flex-1 space-y-1">
                  ...
                  <input 
                    ...
                    value={isEditModalOpen ? editingItem?.nama : newRegistrant.nama} 
                    onChange={e => isEditModalOpen ? setEditingItem({...editingItem!, nama: e.target.value}) : setNewRegistrant({...newRegistrant, nama: e.target.value})} 
                    required 
                  />
                </div>
              </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {['Putra', 'Putri'].map((g) => (
                    <button
                      key={g}
                      type="button"
                      onClick={() => isEditModalOpen ? setEditingItem({...editingItem!, jenis_kelamin: g}) : setNewRegistrant({...newRegistrant, jenis_kelamin: g})}
                      className={`py-2 rounded-xl font-bold text-xs border-2 transition-all ${(isEditModalOpen ? editingItem?.jenis_kelamin : newRegistrant.jenis_kelamin) === g ? 'bg-blue-600 border-blue-600 text-white' : 'bg-slate-50 border-slate-100 text-slate-400'}`}
                    >
                      {g.toUpperCase()}
                    </button>
                  ))}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <input 
                    ...
                    value={isEditModalOpen ? editingItem?.whatsapp : newRegistrant.whatsapp} 
                    onChange={e => isEditModalOpen ? setEditingItem({...editingItem!, whatsapp: e.target.value}) : setNewRegistrant({...newRegistrant, whatsapp: e.target.value})} 
                    required 
                  />
                </div>
                <div className="space-y-1">
                  ...
                  <select 
                    className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl font-bold text-sm" 
                    value={isEditModalOpen ? editingItem?.kategori : newRegistrant.kategori} 
                    onChange={e => isEditModalOpen ? setEditingItem({...editingItem!, kategori: e.target.value}) : setNewRegistrant({...newRegistrant, kategori: e.target.value})}
                  >
                ...
                <div className="col-span-2 space-y-1">
                  ...
                  <input 
                    ...
                    value={isEditModalOpen ? editingItem?.domisili : newRegistrant.domisili} 
                    onChange={e => isEditModalOpen ? setEditingItem({...editingItem!, domisili: e.target.value}) : setNewRegistrant({...newRegistrant, domisili: e.target.value})} 
                    required 
                  />
                </div>
              </div>
