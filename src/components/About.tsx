...
export default function About({ activeTab: propsActiveTab, onTabChange }: AboutProps) {
  ...
  return (
    /* PERBAIKAN: Menambahkan min-h-screen dan flex-col agar pas di layar desktop */
    <section id="tentang-kami" className="relative w-full min-h-screen bg-white py-12 md:py-20 flex flex-col items-center">
      <div className="max-w-7xl mx-auto px-4 w-full">
        ...
        {/* Tab Content Box - Dioptimalkan agar konten di dalamnya fleksibel */}
        <div className="bg-slate-50 rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-12 border border-slate-100 shadow-sm min-h-[500px] flex items-center">
          
          {/* SEJARAH */}
          {activeTab === 'sejarah' && (
            <div id="sejarah" className="w-full animate-in fade-in slide-in-from-bottom-4 duration-500">
              <div className="text-center mb-8 md:mb-12">
                <h3 className="text-2xl md:text-5xl font-black text-slate-900 mb-4 uppercase tracking-tight">
                  {dynamicContent.sejarah_title || "MEMBINA"} <span className="text-blue-600">{dynamicContent.sejarah_accent || "LEGENDA"}</span> MASA DEPAN
                </h3>
                <p className="text-slate-500 text-base md:text-lg max-w-3xl mx-auto leading-relaxed font-medium">
              ...
              <div className="grid lg:grid-cols-2 gap-8 md:gap-12 items-center">
                <div className="relative group overflow-hidden rounded-[2rem]">
                  ...
                  <img 
                    ...
                    className="relative w-full h-[300px] md:h-[400px] object-cover shadow-2xl border-4 border-white transition-transform duration-700 group-hover:scale-110"
                  />
                </div>
                  ...
                  <ul className="space-y-3 md:space-y-4">
                    {[
                      ...
                      <li key={index} className="flex items-start gap-3 text-slate-700 font-semibold group">
                        ...
                        <span className="text-sm md:text-[16px] leading-tight">{item}</span>
                      </li>
                    ))}
                  </ul>
                </div>
          ...
          {/* VISI MISI */}
          {activeTab === 'visi-misi' && (
            <div id="visi-misi" className="w-full animate-in fade-in slide-in-from-bottom-4 duration-500 grid lg:grid-cols-2 gap-6 md:gap-8 items-stretch">
              <div className="bg-white p-8 md:p-10 rounded-3xl shadow-sm border border-slate-100 relative overflow-hidden group flex flex-col justify-center">
                ...
                <ul className="space-y-3 text-slate-700 font-medium">
                  {(dynamicContent.missions || ["Pelatihan terstruktur & disiplin", "Fasilitas berstandar internasional", "Kompetisi rutin berkala", "Pembentukan karakter juara"]).map((misi, i) => (
                    <li key={i} className="flex items-center gap-2 italic">
                      <CheckCircle2 size={16} className="text-blue-500 shrink-0" /> {misi}
                    </li>
                  ))}
                </ul>
              </div>
          ...
          {/* FASILITAS */}
          {activeTab === 'fasilitas' && (
            <div id="fasilitas" className="w-full animate-in fade-in duration-500 grid lg:grid-cols-2 gap-8 md:gap-12 items-center">
              <div>
                <h3 className="text-xl md:text-2xl font-black mb-6 md:mb-8 text-slate-800 uppercase">Fasilitas Standar Nasional</h3>
                <ul className="space-y-4 md:space-y-5">
                  {[
                    ...
                    <li key={index} className="flex items-center gap-4 text-slate-600 text-base md:text-lg font-medium group">
                      <span className="w-8 h-8 bg-white text-blue-600 rounded-lg flex items-center justify-center shadow-sm group-hover:bg-blue-600 group-hover:text-white transition-colors shrink-0">
                        <Shield size={16} />
                      </span>
                      {item}
                    </li>
              ...
              <div className="grid grid-cols-2 gap-3 md:gap-4 h-[300px] md:h-[400px]">
                <img 
                  src="dpnkwabotttfihp7gf3r.jpg" 
                  alt="Badminton Court" 
                  className="w-full h-full object-cover rounded-2xl shadow-md border-2 border-white hover:brightness-110 transition-all"
                />
