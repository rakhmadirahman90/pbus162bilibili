...
export default function About({ activeTab: propsActiveTab, onTabChange }: AboutProps) {
  ...
  // Helper Warna Label sesuai hirarki di gambar
  const getLevelColor = (level: number) => {
    switch(level) {
      case 1: return 'bg-amber-500'; // Penanggung Jawab
      case 2: return 'bg-emerald-500'; // Penasehat
      case 3: return 'bg-blue-600'; // Pembina
      case 4: return 'bg-indigo-600'; // Ketua Umum
      default: return 'bg-slate-500';
    }
  };

  // Helper untuk mengelompokkan anggota berdasarkan Bidang (Role mengandung nama bidang)
  const renderDepartment = (title: string, roleKey: string) => {
    const coordinator = orgData.find(m => m.role.toLowerCase().includes('koordinator') && m.role.toLowerCase().includes(roleKey.toLowerCase()));
    const members = orgData.filter(m => m.role.toLowerCase().includes('anggota') && m.role.toLowerCase().includes(roleKey.toLowerCase()));
    ...
    return (
      <div className="w-full mb-16">
        ...
        {coordinator && (
          <div className="flex flex-col items-center mb-8">
            <div className="bg-white p-4 rounded-[2rem] border-2 border-amber-400 shadow-lg text-center w-48 md:w-56">
              ...
              <span className="text-[7px] bg-amber-500 text-white px-3 py-1 rounded-full font-black uppercase">
                {coordinator.role}
              </span>
            </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 max-w-4xl mx-auto">
          {members.map(p => (
            <div key={p.id} className="bg-white p-3 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-all flex flex-col items-center text-center">
              ...
              <p className="text-blue-500 font-bold text-[6px] md:text-[7px] uppercase mt-1 opacity-80">{p.role}</p>
            </div>
          ))}
        </div>
      </div>
    ...
    <section id="tentang-kami" className="relative w-full h-screen bg-white flex flex-col items-center overflow-hidden font-sans">
      <div className="max-w-7xl mx-auto px-4 w-full h-full flex flex-col py-2 md:py-4">
        ...
        <div className={`flex-1 min-h-0 bg-slate-50/50 rounded-[1.5rem] md:rounded-[2.5rem] p-4 md:p-6 border border-slate-100 shadow-sm relative ${activeTab === 'organisasi' ? 'overflow-y-auto custom-scrollbar' : 'overflow-hidden'}`}>
          ...
          {/* TAB ORGANISASI - SESUAI PREVIEW ADMIN */}
          {activeTab === 'organisasi' && (
            <div className="w-full flex flex-col items-center py-8 animate-in slide-in-from-bottom-5 duration-700 pb-32">
              ...
              <div className="relative flex flex-col items-center mb-12">
                {orgData.filter(m => m.level === 1).map(p => (
                  <div key={p.id} className="relative z-10 flex flex-col items-center">
                    <div className="bg-white p-4 rounded-[2.5rem] border-4 border-amber-400 shadow-2xl text-center w-60 md:w-72 transform transition-all hover:scale-105">
                      <div className="w-28 h-28 md:w-36 md:h-36 mx-auto mb-3">
                        <img 
                          ...
                          className="w-full h-full rounded-[2rem] object-cover border-2 border-slate-50 shadow-md" 
                          alt={p.name}
                        />
                      </div>
                      <p className="font-black text-[11px] md:text-sm uppercase italic text-slate-900 leading-tight mb-2">{p.name}</p>
                      <span className="text-[9px] bg-amber-500 text-white px-5 py-1.5 rounded-full font-black uppercase tracking-tighter">
                        {p.role}
                      </span>
                    </div>
                 ...
                 {orgData.filter(m => m.level === 4).map(p => (
                  <div key={p.id} className="flex flex-col items-center">
                    <div className="bg-blue-600 p-4 rounded-3xl shadow-2xl text-center w-52 md:w-60 transform border-4 border-white">
                      ...
                      <span className="text-[8px] bg-white text-blue-600 px-4 py-1 rounded-full font-black uppercase">
                        {p.role}
                      </span>
                    </div>
              ...
              {/* DINAMIS BERDASARKAN BIDANG (Sesuai Gambar Admin) */}
              <div className="w-full max-w-6xl px-2">
                ...
                {/* Fallback untuk level lain yang tidak masuk kategori bidang diatas */}
                <div className="mt-12">
                  <div className="flex items-center gap-4 mb-8">
                    <div className="h-px flex-1 bg-slate-100"></div>
                    <span className="text-[8px] font-bold text-slate-300 uppercase">Pengurus Lainnya</span>
                    <div className="h-px flex-1 bg-slate-100"></div>
                  </div>
                  <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 gap-3">
                    {orgData.filter(m => m.level >= 5 && 
                      !m.role.toLowerCase().includes('pertandingan') && 
                      !m.role.toLowerCase().includes('binpres') && 
                      !m.role.toLowerCase().includes('humas') && 
                      !m.role.toLowerCase().includes('rohani')
                    ).map(p => (
                      <div key={p.id} className="bg-white/50 p-2 rounded-xl border border-slate-50 text-center">
                        <p className="font-bold text-[7px] uppercase text-slate-600 truncate">{p.name}</p>
                        <p className="text-[6px] text-blue-400 uppercase">{p.role}</p>
                      </div>
                    ))}
                  </div>
                </div>
