...
export default function AdminFooter() {
  ...
  const addNavigation = () => {
    setFooterConfig({
      ...footerConfig,
      navigation: [...footerConfig.navigation, { name: 'Menu Baru', id: 'new-menu' }]
    });
  };
    ...
    <div className="p-6 bg-[#0f111a] min-h-screen text-white space-y-10 font-sans">
      <div className="max-w-5xl mx-auto bg-[#161925] rounded-[2.5rem] border border-white/5 p-8 shadow-2xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="space-y-6">
            {/* NAVIGASI DENGAN TAMBAH/KURANG */}
            <div className="p-5 bg-[#0f111a] rounded-3xl border border-white/5 space-y-4">
              <div className="flex justify-between items-center">
                <label className="text-[10px] font-black uppercase text-blue-500 tracking-[0.2em]">Menu Navigasi</label>
                <button 
                  onClick={addNavigation}
                  className="p-1.5 bg-blue-600/20 hover:bg-blue-600 text-blue-500 hover:text-white rounded-lg transition-all"
                >
                  <Plus size={14} />
                </button>
              </div>
              <div className="space-y-2 max-h-52 overflow-y-auto pr-2 custom-scrollbar">
                {footerConfig.navigation.map((item, idx) => (
                  <div key={idx} className="flex items-center gap-2 group">
                    <div className="flex items-center gap-3 bg-[#161925] flex-1 p-2 rounded-xl border border-white/5 group-hover:border-blue-500/30 transition-all">
                    ...
                    <button onClick={() => removeNavigation(idx)} className="p-2 text-slate-600 hover:text-red-500 transition-colors">
                      <Trash2 size={14} />
                    </button>
                  </div>
                ))}
            ...
            <div className="grid grid-cols-2 gap-4">
               <input placeholder="WhatsApp" className="w-full bg-[#0f111a] border border-white/10 rounded-xl p-3 text-xs outline-none" value={footerConfig.phone} onChange={(e) => setFooterConfig({...footerConfig, phone: e.target.value})} />
               <input placeholder="Email" className="w-full bg-[#0f111a] border border-white/10 rounded-xl p-3 text-xs outline-none" value={footerConfig.email} onChange={(e) => setFooterConfig({...footerConfig, email: e.target.value})} />
            </div>

            <div className="grid grid-cols-2 gap-3">
                <div className="flex items-center gap-3 bg-[#0f111a] p-2 rounded-xl border border-white/5">
                  <Instagram size={14} className="text-pink-500" />
                  <input className="bg-transparent flex-1 text-[10px] outline-none" placeholder="#" value={footerConfig.socials.instagram} onChange={(e) => setFooterConfig({...footerConfig, socials: {...footerConfig.socials, instagram: e.target.value}})} />
                </div>
                <div className="flex items-center gap-3 bg-[#0f111a] p-2 rounded-xl border border-white/5">
                  <Facebook size={14} className="text-blue-500" />
                  <input className="bg-transparent flex-1 text-[10px] outline-none" placeholder="#" value={footerConfig.socials.facebook} onChange={(e) => setFooterConfig({...footerConfig, socials: {...footerConfig.socials, facebook: e.target.value}})} />
                </div>
                <div className="flex items-center gap-3 bg-[#0f111a] p-2 rounded-xl border border-white/5">
                  <Twitter size={14} className="text-sky-400" />
                  <input className="bg-transparent flex-1 text-[10px] outline-none" placeholder="#" value={footerConfig.socials.twitter} onChange={(e) => setFooterConfig({...footerConfig, socials: {...footerConfig.socials, twitter: e.target.value}})} />
                </div>
                <div className="flex items-center gap-3 bg-[#0f111a] p-2 rounded-xl border border-white/5">
                  <Youtube size={14} className="text-red-600" />
                  <input className="bg-transparent flex-1 text-[10px] outline-none" placeholder="#" value={footerConfig.socials.youtube} onChange={(e) => setFooterConfig({...footerConfig, socials: {...footerConfig.socials, youtube: e.target.value}})} />
                </div>
            </div>
          </div>
        </div>
        ...
        <button 
          ...
          disabled={loading}
          className="w-full mt-8 py-4 bg-blue-600 hover:bg-blue-700 text-white rounded-2xl font-black uppercase tracking-[0.3em] text-[10px] flex items-center justify-center gap-3 shadow-xl transition-all"
        >
          {loading ? 'MENYINKRONKAN...' : <><Save size={18} /> SINKRONISASI PERUBAHAN</>}
        </button>
      </div>

      {/* LIVE PREVIEW - PERSIS LANDING PAGE */}
      <div className="max-w-6xl mx-auto space-y-4 pb-20">
        <h3 className="flex items-center gap-2 text-slate-500 font-bold text-[10px] uppercase tracking-widest ml-4">
          <Eye size={14} /> Live Preview
        </h3>
        <div className="bg-[#0a0c14] rounded-[3rem] p-12 border border-white/10 pointer-events-none">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-12 text-left">
            <div className="col-span-1">
              ...
              <p className="text-slate-400 text-[11px] leading-relaxed w-full">{footerConfig.description}</p>
            </div>
            ...
            <div>
              ...
              <div className="flex gap-3">
                ...
                <div className="w-8 h-8 bg-white/5 rounded-full flex items-center justify-center border border-white/5"><Twitter size={12}/></div>
                <div className="w-8 h-8 bg-white/5 rounded-full flex items-center justify-center border border-white/5"><Youtube size={12}/></div>
              </div>
            </div>
          </div>
