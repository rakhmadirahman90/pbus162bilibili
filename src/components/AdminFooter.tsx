...
export default function AdminFooter() {
  ...
  useEffect(() => {
    async function getFooterData() {
      ...
      if (data?.footer_config) {
        // Merge data untuk memastikan field baru (copyright/navigation) tidak kosong
        setFooterConfig({
          ...footerConfig,
          ...data.footer_config,
  ...
  // Fungsi untuk update item navigasi
    ...
    <div className="p-6 bg-[#0f111a] min-h-screen text-white space-y-10">
      <div className="max-w-5xl mx-auto bg-[#161925] rounded-[2.5rem] border border-white/5 p-8 shadow-2xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          {/* KOLOM KANAN: KONTAK, SOSMED & NAVIGASI */}
          <div className="space-y-6">
            ...
            <div className="p-4 bg-[#0f111a] rounded-2xl border border-white/5 space-y-3">
              <label className="text-[10px] font-black uppercase text-blue-500 tracking-[0.2em] block">Menu Navigasi</label>
            ...
            <div className="p-4 bg-[#0f111a] rounded-2xl border border-white/5 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="flex items-center gap-3 bg-[#161925] p-2 rounded-xl border border-white/5">
                  ...
                  <input className="bg-transparent flex-1 text-[10px] outline-none" placeholder="URL" value={footerConfig.socials.instagram} onChange={(e) => setFooterConfig({...footerConfig, socials: {...footerConfig.socials, instagram: e.target.value}})} />
                </div>
                <div className="flex items-center gap-3 bg-[#161925] p-2 rounded-xl border border-white/5">
                  <Facebook size={16} className="text-blue-500" />
                  <input className="bg-transparent flex-1 text-[10px] outline-none" placeholder="URL" value={footerConfig.socials.facebook} onChange={(e) => setFooterConfig({...footerConfig, socials: {...footerConfig.socials, facebook: e.target.value}})} />
                </div>
                <div className="flex items-center gap-3 bg-[#161925] p-2 rounded-xl border border-white/5">
                  <Twitter size={16} className="text-sky-400" />
                  <input className="bg-transparent flex-1 text-[10px] outline-none" placeholder="URL" value={footerConfig.socials.twitter} onChange={(e) => setFooterConfig({...footerConfig, socials: {...footerConfig.socials, twitter: e.target.value}})} />
                </div>
                <div className="flex items-center gap-3 bg-[#161925] p-2 rounded-xl border border-white/5">
                  <Youtube size={16} className="text-red-600" />
                  <input className="bg-transparent flex-1 text-[10px] outline-none" placeholder="URL" value={footerConfig.socials.youtube} onChange={(e) => setFooterConfig({...footerConfig, socials: {...footerConfig.socials, youtube: e.target.value}})} />
                </div>
              </div>
            </div>
          </div>
      ...
      {/* LIVE PREVIEW (Sesuai Gambar Landing Page) */}
      <div className="max-w-6xl mx-auto space-y-4 pb-20">
        <h3 className="flex items-center gap-2 text-slate-500 font-bold text-sm ml-4">
          <Eye size={16} /> LIVE PREVIEW
        </h3>
        <div className="bg-[#0a0c14] rounded-[3rem] p-12 border border-white/10 pointer-events-none opacity-90">
            ...
            <div>
              ...
              <ul className="text-slate-400 text-xs space-y-3 font-medium">
                {footerConfig.navigation.map((nav, idx) => (
                  <li key={idx} className="hover:text-white transition-colors">{nav.name}</li>
                ))}
              </ul>
            </div>
          ...
          <div className="mt-12 pt-8 border-t border-white/5 text-center text-[9px] text-slate-500 font-bold tracking-[0.4em] uppercase">
