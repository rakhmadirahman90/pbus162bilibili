    ...
    <div className="p-8 bg-[#050505] min-h-screen text-white font-sans relative overflow-hidden">
      ...
      <div className="grid gap-4 mb-8">
        {loading && atlets.length === 0 ? (
          <div className="py-24 flex flex-col items-center gap-4">
            ...
            <p className="text-zinc-600 font-bold text-xs uppercase tracking-widest italic">Syncing Database...</p>
          </div>
        ) : (
                ...
                <div className="flex items-center gap-10 mt-6 md:mt-0">
                  <div className="text-right">
                    ...
                    <p className="text-4xl font-black text-white leading-none">
                      {(atlet.display_points || 0).toLocaleString()} <span className="text-blue-600 text-sm italic">PTS</span>
                    </p>
                  </div>
              ...
              {expandedId === atlet.id && (
                <div className="bg-zinc-950 border-x border-b border-blue-600/30 rounded-b-[2.5rem] p-6 animate-in slide-in-from-top-4 duration-300">
                  ...
                  <div className="space-y-3">
                    <p className="text-[10px] text-zinc-600 font-black uppercase">Recent Activity</p>
                    {histories.length > 0 ? histories.map((log) => (
                      <div key={log.id} className="flex items-center justify-between text-[11px] bg-zinc-900/50 p-3 rounded-xl border border-white/5">
                        ...
                        <span className={`font-bold ${log.perubahan > 0 ? 'text-green-400' : 'text-red-400'}`}>
                          {log.perubahan > 0 ? '+' : ''}{log.perubahan} PTS
                        </span>
                      </div>
                    )) : (
                      <p className="text-center text-zinc-600 text-[10px] py-4 italic uppercase">No History Found.</p>
                    )}
                  </div>
                </div>
              )}
      ...
      <div className={`fixed bottom-10 left-1/2 -translate-x-1/2 z-[100] transition-all duration-700 transform ${showSuccess ? 'translate-y-0 opacity-100' : 'translate-y-24 opacity-0 pointer-events-none'}`}>
        <div className="bg-emerald-600 px-10 py-6 rounded-full shadow-2xl flex items-center gap-4 border border-white/20">
          ...
          <h4 className="text-white font-black uppercase text-lg italic tracking-widest">SYNC SUCCESSFUL!</h4>
        </div>
      </div>
    </div>
  );
