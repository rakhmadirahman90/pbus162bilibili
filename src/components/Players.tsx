...
// --- DATA SOURCE & DAFTAR PEMENANG ---
const EVENT_LOG = [
  { 
    id: 1, 
    // Nama-nama di bawah ini otomatis mendapatkan bonus +300 PTS
    winners: [
...
const Players: React.FC = () => {
  ...
  // 3. PROSES DATA: POIN SINKRON & URUTAN RANKING
  const processedPlayers = useMemo(() => {
    const config: Record<string, any> = {
      // Menyesuaikan kunci dengan isi kolom "kategori" di database Anda
    ...
    const defaultConfig = { base: 4500, label: 'UNSEEDED', age: 'Senior' };

    return dbPlayers.map((p) => {
      // Normalisasi kategori dari DB agar cocok dengan config
      const playerCat = p.kategori ? p.kategori.toUpperCase() : '';
      const conf = config[playerCat] || defaultConfig;
      
      // Bonus Pemenang Turnamen Internal (+300 PTS)
      const isWinner = EVENT_LOG[0].winners.includes(p.nama);
      const winBonus = isWinner ? 300 : 0;
      
      // Kalkulasi: Base Kategori + Bonus Menang
      // Contoh Agustilaar: 10.000 + 300 = 10.300 PTS
      ...
      return {
        ...
        name: p.nama,
        img: p.foto_url,
        bio: p.pengalaman || "Atlet profesional PB US 162.",
        totalPoints,
        isWinner,
        ageGroup: conf.age, // Untuk filter tombol Senior/Muda
            ...
            <div className="p-10 md:p-16 flex-1 bg-zinc-900 relative">
              ...
              <div className="grid grid-cols-2 gap-5">
                <div className="bg-white/2 p-6 rounded-[2rem] border border-white/5 text-center">
                    <Zap className="text-blue-500 mb-2 mx-auto" size={20} />
                ...
                <div className="bg-white/2 p-6 rounded-[2rem] border border-white/5 text-center">
                    <Trophy className="text-yellow-500 mb-2 mx-auto" size={20} />
              ...
              {filteredPlayers.map((player) => (
                <SwiperSlide key={player.id}>
                  ...
                  >
                    ...
                    {/* Badge Nomor Ranking Berdasarkan Poin */}
