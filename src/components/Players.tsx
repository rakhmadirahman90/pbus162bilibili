...
const Players: React.FC<PlayersProps> = ({ initialFilter = 'Semua' }) => {
  ...
  // --- FIX UTAMA: FORCE UPDATE DAN SLIDE TO ZERO ---
  useEffect(() => {
    if (swiperInstanceRef.current && filteredPlayers.length > 0) {
      // Pemicu update berkala untuk memastikan render sinkron dengan Swiper
      const timer = setTimeout(() => {
        swiperInstanceRef.current.update();
        if (swiperInstanceRef.current.activeIndex > 0) {
        ...
        {isLoading ? (
          ...
          <div className="relative group/slider">
            {filteredPlayers.length > 0 ? (
              <Swiper
                modules={[Navigation, Pagination, Autoplay, Observer]} // Modul Observer diaktifkan
                onSwiper={(swiper) => {
                  swiperInstanceRef.current = swiper;
                }}
                key={`swiper-v4-${currentAgeGroup}-${filteredPlayers.length}`} // Key unik diperkuat
                spaceBetween={25}
                slidesPerView={1.2}
                observer={true} // Swiper akan mendeteksi perubahan DOM anak
                observeParents={true} // Swiper mendeteksi jika kontainer berubah
                watchSlidesProgress={true} // Memastikan progress slide terhitung meski render asinkron
