...
// --- DAFTAR PEMENANG TURNAMEN INTERNAL (SESUAI TABEL POIN +300) ---
const EVENT_LOG = [
  { 
    id: 1, 
    winners: [
...
const Players: React.FC = () => {
  ...
  const fetchPlayersFromDB = async () => {
    try {
      // Mengambil semua kolom termasuk 'pengalaman' untuk deskripsi
  ...
  const processedPlayers = useMemo(() => {
    ...
    return dbPlayers.map((p) => {
      ...
      return {
        ...
        name: p.nama,
        img: p.foto_url,
        // DESKRIPSI: Mengambil dari kolom 'pengalaman' di database
        bio: p.pengalaman && p.pengalaman !== "ok" && p.pengalaman !== "Ok" 
             ? p.pengalaman 
    ...
    // URUTAN: Selalu yang tertinggi poinnya di posisi pertama
        ...
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4">
          ...
          <div className="relative bg-zinc-900 border border-white/10 w-full max-w-5xl rounded-[3rem] overflow-hidden flex flex-col md:flex-row shadow-2xl">
            ...
            <div className="p-10 md:p-16 flex-1 bg-zinc-900 relative">
              ...
              {/* AREA DESKRIPSI PROFIL */}
              <div className="mb-10">
                <p className="text-zinc-500 text-xs font-black uppercase tracking-widest mb-3 flex items-center gap-2">
                    <Info size={14} /> Biografi Atlet
                </p>
                <p className="text-zinc-300 text-lg leading-relaxed">{selectedPlayer.bio}</p>
              </div>
        ...
        <div className="flex gap-4 mb-16">
          {['Semua', 'Senior', 'Muda'].map((tab) => (
            <button 
              ...
              className={`px-10 py-4 rounded-full text-[11px] font-black transition-all ${currentAgeGroup === tab ? 'bg-blue-600 text-white shadow-xl shadow-blue-600/20' : 'text-zinc-500 bg-zinc-900 border border-zinc-800 hover:text-white'}`}
            >
              {tab.toUpperCase()}
            </button>
          ))}
              ...
              {filteredPlayers.map((player) => (
                <SwiperSlide key={player.id}>
                  <div 
                    ...
                    className="group cursor-pointer relative aspect-[3/4.5] rounded-[3.5rem] overflow-hidden bg-zinc-900 border border-zinc-800 hover:border-blue-600 transition-all duration-700 hover:-translate-y-4"
                  >
                    ...
                    {/* Badge Ranking Berdasarkan Poin */}
                    ...
                    {player.isWinner && (
                      <div className="absolute top-8 right-8 bg-yellow-500 p-2.5 rounded-xl text-black shadow-lg">
                        <Trophy size={18} />
                      </div>
                    )}

                    <div className="absolute bottom-10 left-10 right-10">
                      <p className="text-zinc-500 text-[10px] font-black uppercase mb-1 tracking-[0.2em]">{player.ageGroup}</p>
