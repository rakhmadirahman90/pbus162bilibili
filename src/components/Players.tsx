...
import { 
  ...
  ArrowRight, LayoutDashboard, Search // Tambahkan Search icon
} from 'lucide-react';

// ... (Interface dan playersData tetap sama seperti sebelumnya)
...
export default function Players() {
  ...
  // Update logika filter untuk mendukung pencarian nama
  ...
  // PlayerModal component tetap sama...
  const PlayerModal = () => {
    ...
    return (
      <div className="fixed inset-0 z-[999] flex items-center justify-center p-4 md:p-8">
        ...
        <div className="relative bg-[#121212] border border-white/10 w-full max-w-4xl rounded-[2.5rem] overflow-hidden shadow-2xl flex flex-col md:flex-row animate-in zoom-in duration-300">
          <button onClick={() => setSelectedPlayer(null)} className="absolute top-4 right-4 z-10 bg-black/50 hover:bg-blue-600 p-2 rounded-full text-white transition-all shadow-lg">
            <X size={24} />
          </button>
          ...
          <div className="w-full md:w-1/2 p-8 md:p-12 flex flex-col justify-center">
            ...
            <h2 className="text-4xl md:text-5xl font-black uppercase mb-2 leading-none">{selectedPlayer.name}</h2>
            <p className="text-zinc-500 font-medium mb-8">Pilar Utama PB US 162 - {selectedPlayer.ageGroup}</p>
  ...
  return (
    <section id="atlet" className="py-24 bg-[#0a0a0a] text-white min-h-screen relative">
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end mb-16 gap-8 border-b border-white/5 pb-12">
          <div className="space-y-6 w-full lg:w-auto">
            ...
            <div className="relative group max-w-md">
              ...
              <input 
                ...
                className="w-full bg-zinc-900/50 border border-zinc-800 rounded-2xl py-4 pl-12 pr-4 focus:outline-none focus:border-blue-600 focus:ring-1 focus:ring-blue-600 transition-all text-sm"
              />
              {searchTerm && (
                <button 
                  onClick={() => setSearchTerm("")}
                  className="absolute right-4 inset-y-0 flex items-center text-zinc-500 hover:text-white"
                >
                  <X size={16} />
                </button>
              )}
          ...
          <div className="flex bg-zinc-900/80 backdrop-blur-md p-1.5 rounded-2xl border border-zinc-800 shadow-xl">
            <button onClick={() => { setCurrentTab('Atlet Senior'); setShowAll(false); }} className={`flex items-center gap-3 px-6 md:px-8 py-4 rounded-xl font-black text-xs tracking-widest transition-all ${currentTab === 'Atlet Senior' ? 'bg-blue-600 text-white' : 'text-zinc-500 hover:text-white'}`}>
              <UserCheck size={18} /> SENIOR
            </button>
            <button onClick={() => { setCurrentTab('Atlet Muda'); setShowAll(false); }} className={`flex items-center gap-3 px-6 md:px-8 py-4 rounded-xl font-black text-xs tracking-widest transition-all ${currentTab === 'Atlet Muda' ? 'bg-blue-600 text-white' : 'text-zinc-500 hover:text-white'}`}>
              <Users size={18} /> MUDA
            </button>
          </div>
        </div>
        ...
        {filteredPlayers.length === 0 ? (
          <div className="py-20 text-center">
            <Search size={48} className="mx-auto text-zinc-700 mb-4 opacity-20" />
            <p className="text-zinc-500 font-bold uppercase tracking-widest">Pemain "{searchTerm}" tidak ditemukan</p>
          </div>
        ) : (
          /* SLIDER / GRID LOGIC */
          !showAll ? (
            <div className="relative group">
              <Swiper
                key={`${currentTab}-${searchTerm}`} // Key ditambahkan agar swiper reset saat cari/ganti tab
              ...
              <button ref={prevRef} className="absolute left-0 top-[40%] -translate-y-1/2 z-40 w-12 h-12 rounded-full bg-blue-600 flex items-center justify-center opacity-0 group-hover:opacity-100 -translate-x-6 transition-all hover:scale-110"><ChevronLeft size={24} /></button>
              <button ref={nextRef} className="absolute right-0 top-[40%] -translate-y-1/2 z-40 w-12 h-12 rounded-full bg-blue-600 flex items-center justify-center opacity-0 group-hover:opacity-100 translate-x-6 transition-all hover:scale-110"><ChevronRight size={24} /></button>
            </div>
          ) : (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
              {filteredPlayers.map((player) => (
                <div key={player.id} onClick={() => setSelectedPlayer(player)} className="group/grid cursor-pointer relative aspect-[3/4.5] rounded-3xl overflow-hidden bg-zinc-900 border border-zinc-800 hover:border-blue-600 transition-colors">
                  <img src={player.image} alt={player.name} className="w-full h-full object-cover opacity-50 group-hover:opacity-80 transition-opacity" />
                  <div className="absolute bottom-4 left-4 right-4">
                    <h3 className="text-sm md:text-base font-black uppercase leading-none group-hover:text-blue-400">{player.name}</h3>
