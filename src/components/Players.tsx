...
const Players: React.FC<PlayersProps> = ({ initialFilter = 'Semua' }) => {
  ...
  const fetchPlayersFromDB = async () => {
    try {
      ...
      const { data: winnersData } = await supabase.from('hasil_turnamen').select('nama_atlet');
  ...
  const processedPlayers = useMemo(() => {
    const ageMapping: Record<string, string> = {
      'SENIOR': 'Senior', 'DEWASA': 'Senior', 'DEWASA / UMUM': 'Senior',
      'TARUNA (U-19)': 'Muda', 'REMAJA (U-17)': 'Muda', 'PEMULA (U-15)': 'Muda',
    };
  ...
  // --- FIX: FORCE UPDATE SWIPER KETIKA LOADING SELESAI ---
  useEffect(() => {
    if (swiperRef.current && filteredPlayers.length > 0) {
  ...
  return (
    <section id="atlet" className="py-24 bg-[#050505] text-white min-h-screen relative overflow-hidden font-sans">
      {/* MODAL DETAIL (Disingkat untuk menghemat ruang) */}
      {selectedPlayer && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/95 backdrop-blur-2xl" onClick={() => setSelectedPlayer(null)} />
          <div className="relative bg-zinc-900 border border-white/10 w-full max-w-5xl rounded-[3rem] overflow-hidden flex flex-col md:flex-row animate-in fade-in zoom-in duration-300">
             <div className="w-full md:w-1/2 h-[400px] md:h-auto relative">
                {selectedPlayer.img ? <img src={selectedPlayer.img} className="w-full h-full object-cover" /> : <div className="w-full h-full bg-zinc-800 flex items-center justify-center"><User size={100} /></div>}
             </div>
             <div className="p-10 flex-1 relative">
                ...
                <h2 className="text-5xl font-black uppercase italic mb-6">{selectedPlayer.name}</h2>
                <div className="grid grid-cols-2 gap-4">
                   <div className="bg-white/5 p-6 rounded-3xl"><p className="text-[10px] text-zinc-500 uppercase font-black">Rank</p><p className="text-2xl font-black">#{processedPlayers.findIndex(p => p.id === selectedPlayer.id) + 1}</p></div>
                   <div className="bg-white/5 p-6 rounded-3xl"><p className="text-[10px] text-zinc-500 uppercase font-black">Poin</p><p className="text-2xl font-black">{selectedPlayer.totalPoints.toLocaleString()}</p></div>
                </div>
             </div>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <div className="flex flex-col md:flex-row justify-between items-end gap-8 mb-12">
          <div>
            ...
            <p className="text-zinc-500 text-xs font-bold uppercase tracking-widest">Data Atlet Terupdate & Realtime</p>
          </div>
        ...
        {/* Filter Tabs */}
        <div className="flex bg-zinc-900/50 p-2 rounded-[2.5rem] border border-zinc-800 w-fit mb-16">
          {['Semua', 'Senior', 'Muda'].map((label) => {
            const count = label === 'Semua' ? dbPlayers.length : processedPlayers.filter(p => p.ageGroup === label).length;
            return (
              <button key={label} onClick={() => setCurrentAgeGroup(label)} className={`px-10 py-4 rounded-[2rem] text-[12px] font-black transition-all ${currentAgeGroup === label ? 'bg-blue-600 text-white shadow-xl' : 'text-zinc-500'}`}>
                {label.toUpperCase()} <span className="ml-2 opacity-40">{count}</span>
              </button>
            );
          })}
        </div>
        ...
        {isLoading ? (
          ...
          <div className="relative group/slider">
            {/* FIX: Hanya render Swiper JIKA filteredPlayers sudah terisi (> 0) */}
            {filteredPlayers.length > 0 ? (
              <Swiper
                modules={[Navigation, Pagination, Autoplay, Observer]}
                onSwiper={(swiper) => (swiperRef.current = swiper)}
                // KEY UNIK: Memaksa Swiper reset total saat data berubah dari 0 ke 54
                key={`atlet-swiper-${filteredPlayers.length}-${currentAgeGroup}`}
                spaceBetween={25}
                slidesPerView={1.2}
                observer={true}
                observeParents={true}
                ...
                {filteredPlayers.map((player) => (
                  <SwiperSlide key={player.id}>
                    <div onClick={() => setSelectedPlayer(player)} className="group cursor-pointer relative aspect-[3/4.5] rounded-[3.5rem] overflow-hidden bg-zinc-900 border border-zinc-800 hover:border-blue-600/50 transition-all duration-700 hover:-translate-y-4">
                      {player.img ? <img src={player.img} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center bg-zinc-800"><User size={60} className="text-zinc-700" /></div>}
              ...
              <div className="py-20 text-center text-zinc-600 font-black uppercase text-xs tracking-widest">
            ...
            <button ref={nextRef} className="absolute right-[-20px] top-1/2 -translate-y-1/2 z-40 w-14 h-14 rounded-full bg-zinc-900 border border-zinc-700 flex items-center justify-center opacity-0 group-hover/slider:opacity-100 hover:bg-blue-600 transition-all shadow-2xl"><ChevronRight size={24} /></button>
          </div>
        )}
      </div>
    </section>
